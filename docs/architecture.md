...
| Payload processing | unprocessed (raw) and undetected (compressed) product counts; compression and detection progress | log(1+n) / log(1+Jetson capacity in raw or compressed products), clipped at one; fraction of job duration |
...
the resulting observation. EventSat trace v5 uses the 45 observations above, 25 state
