...
- representation: rules, PPO reinforcement learning (`rl`), LLM single-shot/agentic variants, hybrid safety-shielded LLM variants, or LeWM-CEM planning;
...
EventSat is the single-spacecraft scheduling benchmark. SSA is a prototype for a later constellation-custody study with sensing- and link-gated information flow. `rl` is an RLlib PPO policy trained per coordinate and deployed onboard on the OBC; `hrl` remains reserved.
